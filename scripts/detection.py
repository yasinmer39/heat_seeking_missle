...
# Add a queue to handle frames for display
frame_queue = queue.Queue()

# Thread function to display frames
def display_frames():
    while True:
...
class KCFTracker:
    ...
    def update_trackers(self, frame, detections):
        ...
        for detection in detections:
            ...
            if label == "person" and confidence > 0.5:  # Example threshold
                x, y, w, h = int(bbox.xmin), int(bbox.ymin), int(bbox.width), int(bbox.height)
                if label not in self.trackers:
                    tracker = cv2.TrackerKCF_create()
                    tracker.init(frame, (x, y, w, h))
                    updated_trackers[label] = tracker
                else:
                    success, new_bbox = self.trackers[label].update(frame)
                    if success:
                        updated_trackers[label] = self.trackers[label]
...
def app_callback(pad, info, user_data):
    ...
    if user_data.use_frame and frame is not None:
        ...
        # Add the frame to the queue for display
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Send exit signal to the display thread
